...
const MODEL_NAME = 'penal-codigo';

/**
 * @swagger
 * /penalCodigo:
 *   get:
 *     summary: Retorna o Código Penal
 *     description: Obtém o conteúdo completo do Código Penal Brasileiro (Decreto-Lei nº 2.848/1940) com URL de referência e data do último scraping
 *     tags:
 *       - Penal
 *     responses:
 *       200:
 *         description: Código Penal recuperado com sucesso
 *         content:
 *           application/json:
 *             schema:
...
penalCodigo.get('/', async (req, res) => {
    try {
        const dados = await PenalCodigo.findOne({ where: { id: 1 } });
        if (!dados) {
 ...
 * /penalCodigo/status:
 *   get:
 *     summary: Status do Código Penal
 *     description: Retorna informações sobre o último scraping e status dos dados do Código Penal Brasileiro
 *     tags:
 *       - Penal
 *     responses:
 *       200:
 ...
 *               $ref: '#/components/schemas/Erro'
 */
penalCodigo.get('/status', async (req, res) => {
    try {
        const dados = await PenalCodigo.findOne({ where: { id: 1 } });
        res.status(200).json({
            exists: !!dados,
...
/**
 * @swagger
 * /penalCodigo/scrape-agora:
 *   post:
 *     summary: Executar scraping imediato
 *     description: Inicia um scraping imediato do Código Penal Brasileiro (Decreto-Lei nº 2.848/1940) de forma assíncrona
 *     tags:
 *       - Penal
 *     responses:
 *       200:
 ...
 *               $ref: '#/components/schemas/Erro'
 */
penalCodigo.post('/scrape-agora', async (req, res) => {
    try {
        const result = await scrapingService.scrapeUrl(URL, PenalCodigo, MODEL_NAME);
        if (result.success) {
            res.status(200).json({ 
