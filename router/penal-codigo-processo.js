...
async function scraping() {
    try {
        ...
        const [registro, criado] = await PenalCodigoProcesso.findOrCreate({
...
penalCodigoProcesso.get('/', async (req, res) => {
    try {
        ...
        const dados = await PenalCodigoProcesso.findOne({ where: { id: 1 } }); // Altere o critério conforme necessário
