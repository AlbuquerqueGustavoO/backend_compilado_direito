...
const MODEL_NAME = 'administrativo-servicos-publico';

/**
 * @swagger
 * /administrativoServicosPublicos:
 *   get:
 *     summary: Retorna a Lei de Concessões e Permissões de Serviços Públicos
 *     description: Obtém o conteúdo completo da Lei nº 8.987/1995 com URL de referência e data do último scraping
 *     tags:
 *       - Administrativo
 *     responses:
 *       200:
 *         description: Lei de Concessões e Permissões de Serviços Públicos recuperada com sucesso
 *         content:
 *           application/json:
 *             schema:
...
/**
 * @swagger
 * /administrativoServicosPublicos/status:
 *   get:
 *     summary: Status da Lei de Concessões e Permissões de Serviços Públicos
 ...
 *               $ref: '#/components/schemas/Erro'
 */
administrativoServicosPublicos.get('/status', async (req, res) => {
    try {
        const dados = await AdministrativoServicosPublicos.findOne({ where: { id: 1 } });
        res.status(200).json({
            exists: !!dados,
...
/**
 * @swagger
 * /administrativoServicosPublicos/scrape-agora:
 *   post:
 *     summary: Executar scraping imediato
 *     description: Inicia um scraping imediato da Lei nº 8.987/1995 de forma assíncrona
 *     tags:
 *       - Administrativo
 *     responses:
 *       200:
 ...
 *               $ref: '#/components/schemas/Erro'
 */
administrativoServicosPublicos.post('/scrape-agora', async (req, res) => {
    try {
        const result = await scrapingService.scrapeUrl(URL, AdministrativoServicosPublicos, MODEL_NAME);
        if (result.success) {
            res.status(200).json({ 
